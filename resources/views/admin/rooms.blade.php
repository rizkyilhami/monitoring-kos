...
<head>
    ...
    <title>Room Management - SmarthCost Admin</title>
        ...
        <aside class="panel sidebar">
            <div>
                ...
                <nav>
                    ...
                    <a href="{{ route('admin.rooms') }}" class="nav-link active">
                        <span class="nav-icon">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"
                                stroke-linecap="round" stroke-linejoin="round">
            ...
            <div class="text-sm text-slate-400">View ringkas dan lebih nyaman untuk dipakai.</div>
        </aside>
        <main class="panel content">
            <section class="hero">
                <div class="text-xs uppercase tracking-[0.22em] text-slate-400">Room Management</div>
                <h1 class="text-xl font-semibold mt-1">Kelola status kamar secara cepat</h1>
                <p class="text-sm text-slate-300 mt-1">Tampilan ringkas, rapi, dan lebih ringan agar admin tidak perlu
                    scroll terlalu panjang.</p>
            </section>
            <section class="grid">
                @foreach ($rooms as $room)
                    <div class="card">
                        <div class="flex items-center justify-between gap-3">
                            <div>
                                <div class="mini">{{ $room['name'] }}</div>
                                <div class="font-semibold">Status: <span class="pill">{{ $room['status'] }}</span>
                                </div>
                            </div>
                            <div class="text-sm text-slate-400">Aktif</div>
                        </div>
                        <div class="mt-3 grid grid-cols-2 gap-2 text-sm text-slate-300">
                            <div class="card bg-slate-900/50">Tegangan: {{ $room['voltage'] }}V</div>
                            <div class="card bg-slate-900/50">Arus: {{ $room['current'] }}A</div>
                            <div class="card bg-slate-900/50">Daya: {{ $room['power'] }}W</div>
                            <div class="card bg-slate-900/50">Tagihan: Rp {{ $room['bill'] }}</div>
                        </div>
                    </div>
                @endforeach
            </section>
